	...
	for i := 0; i < t.NumField(); i++ {
		if t.Field(i).Type.String() == "*pbc.Params" {
			...
			data[t.Field(i).Name]=elemStr
				...
				opkpart:=&struct{OPK string;APKMap map[string]string}{opk,apkmap}
				...
				otherStr:=make([]string,len(others))
					...
					ask:=v3.ASK.String()
					askpart:=&struct{F []string;OthersShare []string;YPart string;ASK string}{afStr,aotherStr,ypart,ask}
					raw1,_:=json.Marshal(askpart)
                                        askmap[k3]=string(raw1)

                                }
                                oskpart:=&struct{AlphaPart string;ASKMap map[string]string;F []string;N int;T int;OthersShare []string;OSK string;GOSK string}{alphapart,askmap,fStr,n,t,otherStr,osk,gosk}
		...
		if t.Field(i).Type.String() == "*big.Int" {
			...
			tem=c1s
				...
				gy,_:=helper.G.NewFieldElement().SetString(v.(string),10)
                        ...
                        for k,v :=range raw {
				value:=&struct{OPK string;APKMap map[string]string}{}
				...
				for k1,v1 := range value.APKMap {
					...
					apkmap[k1]=apk
                        ...
                        for k,v :=range raw {
                                value:=&struct{AlphaPart string;ASKMap map[string]string;F []string;N int;T int;OthersShare []string;OSK string;GOSK string}{}
                                _=json.Unmarshal([]byte(v.(string)),value)
                                //fmt.Println(reflect.TypeOf(v))
				...
				t:=int(value.T)
                                ...
                                for k3,v3 := range value.ASKMap {
                                        askpart:=&struct{F []string;OthersShare []string;YPart string;ASK string}{}
